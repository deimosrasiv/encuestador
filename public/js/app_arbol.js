...
//console.log(fechaingreso);
  ...
  db.collection("preg_arbolito").where("estado", "==", 1).onSnapshot((querySnapshot)=> {
   var contador = 1;
...
var horaingreso = (hora1+":"+minutos1+":"+segundos1)
var fechaingreso =(`${year}-`+ month1 + "-" + day1)
//console.log(fechaingreso);
...
var notafinalformateada =Number(notafinal);
...
if (notafinal < 0){
    ...
    db.collection("notas_alimentacion").add({
      nota_promedio: notafinalformateada,
      fecha: fechaingreso,
      hora: horaingreso,
      modulo: "arbolito"
    })
...
if (cuentanota == 0){
  ...
  var mensajemodal = "Esta es la Nota, a la unica pregunta que has evaluado";
...
document.getElementById('enlasemodal').style.display = 'block';
  document.getElementById('nota').style.display = 'block';
